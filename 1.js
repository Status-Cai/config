...
function notifyDone(title, body) {
  // Loon 使用 $notification.post 进行系统通知
...
if (typeof $request !== 'undefined' && $request) {
  ...
  // Loon 使用 $persistentStore.write 写入数据
  ...
  // Loon 使用 $persistentStore.read 读取数据
  ...
  if (!raw) {
    ...
    // Loon 使用 $httpClient 发起网络请求，这里封装成 Promise 以兼容原脚本逻辑
      ...
      return new Promise((resolve, reject) => {
        ...
        $httpClient.get(req, (error, response, body) => {
          if (error) {
            ...
            resolve({ body: body, status: response.status });
          }
        });
      });
    }
